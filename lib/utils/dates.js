...
var dates = _extends({}, _dateArithmetic2.default, {
  ...
  firstVisibleDay: function firstVisibleDay(date, culture, infinityScroll) {
    var beginDate = _dateArithmetic2.default.subtract(date, infinityScroll ? 2 : 0, 'month');
    var firstOfMonth = dates.startOf(beginDate, 'month');

    return dates.startOf(firstOfMonth, 'week', _localizer2.default.startOfWeek(culture));
  },
  lastVisibleDay: function lastVisibleDay(date, culture, infinityScroll) {
    var endDate = _dateArithmetic2.default.add(date, infinityScroll ? 2 : 0, 'month');
    var endOfMonth = dates.endOf(endDate, 'month');

    return dates.endOf(endOfMonth, 'week', _localizer2.default.startOfWeek(culture));
  },
  visibleDays: function visibleDays(date, culture, infinityScroll) {
    var current = dates.firstVisibleDay(date, culture, infinityScroll),
