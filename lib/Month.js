...
var MonthView = function (_React$Component) {
  ...
  MonthView.prototype.render = function render() {
    var _props = this.props,
        ...
        infinityScroll = _props.infinityScroll,
        month = _dates2.default.visibleDays(date, culture, infinityScroll),
    ...
    return _react2.default.createElement(
      ...
      _react2.default.createElement(
        'div',
        {
          ...
          ref: 'monthHolder',
          className: (0, _classnames2.default)('rbc-month-view', className, infinityScroll ? 'rbc-month-view-infinite' : '')
        },
  ...
  MonthView.prototype.measureRowLimit = function measureRowLimit() {
    this.setState({
      ...
      rowLimit: 10 //this.refs.slotRow.getRowLimit(),
    });
  };
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.scrollToCenter = function () {
    var month = _this4.refs.monthHolder;
    var rows = month.getElementsByClassName('rbc-month-row');
  ...
  this.onScroll = function (e) {
    _this4.checkScroll(e.target);
  };

  this.checkScroll = (0, _debounce2.default)(function (node) {
    var scrollPos = node.scrollTop;
    var rows = node.getElementsByClassName('rbc-month-row');
    var rowHeight = rows[0].offsetHeight;
    var nodeHeight = node.offsetHeight;
    var diff = nodeHeight - rowHeight;
    var totalRowHeight = rows.length * rowHeight;

    if (scrollPos < diff) {
      _this4.props.onNavigate('PREV');
      _this4.setState({
        scrollY: scrollPos + diff
      });
    } else if (scrollPos > totalRowHeight - nodeHeight * 2) {
      _this4.props.onNavigate('NEXT');
      _this4.setState({
        scrollY: scrollPos - diff
      });
    }
  ...
  this.renderWeek = function (week, weekIdx) {
    var _props2 = _this4.props,
        ...
        infinityScroll = _props2.infinityScroll;
    ...
    var position = Math.floor(_dateArithmetic2.default.diff(week[0], date, 'day') / 7).toString();

    var rowSpecialClass = rowClassMap[position] || '';

    return _react2.default.createElement(_DateContentRow2.default, {
      key: weekIdx,
      ref: weekIdx === 0 ? 'slotRow' : undefined,
      container: _this4.getContainer,
      className: (0, _classnames2.default)('rbc-month-row', infinityScroll ? 'rbc-month-row-infinite' : '', rowSpecialClass),
      getNow: getNow,
      date: date,
      range: week,
      events: events,
...
MonthView.range = function (date, _ref2) {
  ...
  var infinityScroll = undefined.props.infinityScroll;
  var start = _dates2.default.firstVisibleDay(date, culture, infinityScroll);
  var end = _dates2.default.lastVisibleDay(date, culture, infinityScroll);
